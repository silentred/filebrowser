...
func printUsers(users []*types.User) {
	...
	fmt.Fprintln(w, "ID\tUsername\tScope\tLocale\tV. Mode\tAdmin\tExecute\tCreate\tRename\tModify\tDelete\tPublish\tDownload")
...
func addUserFlags(cmd *cobra.Command) {
	...
	cmd.Flags().Bool("perm.publish", true, "publish perm for users")
...
func getUserDefaults(cmd *cobra.Command, defaults *types.UserDefaults, all bool) {
	visit := func(flag *pflag.Flag) {
		...
		case "perm.publish":
			defaults.Perm.Publish = mustGetBool(cmd, "perm.publish")
		case "perm.download":
